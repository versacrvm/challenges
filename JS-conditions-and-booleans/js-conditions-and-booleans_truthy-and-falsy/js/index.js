document.querySelectorAll('input[type="radio"]').forEach((element) => {
  element.addEventListener("click", (event) => {
    ...
    const truthyLabel = document.querySelector(
      `label[for="truthy-${questionName}"]`
    );
    const falsyLabel = document.querySelector(
      `label[for="falsy-${questionName}"]`
    );

    const clickedLabel = document.querySelector(
      `label[for="${clickedRadioButton.id}"]`
    );
    ...
    const correctAnswer = !!eval(
      clickedLabel.closest("fieldset").querySelector("legend").textContent
    );
